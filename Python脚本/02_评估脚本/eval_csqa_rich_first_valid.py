...
LEADING_CHOICE_PATTERN = re.compile(r"^[\s\"'`]*[\(\[]?\s*([A-E])(?=[^A-Z]|$)")
...
def extract_choice(text: str) -> str:
    ...
    # 1. Prefer explicit answer templates such as:
    #    "The correct answer is C." / "Answer: B"
    for pattern in ANSWER_PATTERNS:
        match = pattern.search(upper_text)
        if match:
            return match.group(1)

    # 2. If the output begins with a valid option letter and then continues with
    #    noise, trust that leading answer. Do not scan the full text for any
    #    standalone A-E token, otherwise normal English articles like "a" will
    #    be misread as option A.
    match = LEADING_CHOICE_PATTERN.match(upper_text)
    if match:
        return match.group(1)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Rich evaluation for CSQA-style multiple-choice generation using the first valid answer letter."
    )
